...
# `python3 profile/sync/generate.py`, which puts the script's own directory on
# sys.path[0] instead of the repo root. An absolute ``from profile.sync import ...``
...
# argv[0] = profile/sync/<name>.py) so a regression in that mode is caught
# here, not in CI.

PROFILE_ROOT = Path(__file__).resolve().parents[1]
REPO_ROOT = PROFILE_ROOT.parent


def _run(script: str, *args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, str(PROFILE_ROOT / "sync" / script), *args],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
        timeout=60,
