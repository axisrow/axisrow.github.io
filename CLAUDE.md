...
- **Exactly seven** `data-effect` canvases exist, pinned in document order: `metaballs` (hero), `starfield` (stars), `plasma` (projects), `mandelbrot` (opensource), `feedback` (experience), `rotozoom` (about), `copperBars` (contact). Forbidden: GSAP, ScrollTrigger, vendor bundles.
